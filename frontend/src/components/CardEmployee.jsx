...
function CardEmployee({ employeesData, handleClickEmployee }) {
    return (
        <main className='absolute flex flex-col w-full items-center mt-28'>
            <div className='flex justify-center text-center text-white text-6xl font-bold w-2/4'>
                Arrive register
            </div>
            <div className='flex flex-col justify-center items-center mt-8 h-[calc(70vh-110px)] w-[50%]'>
                {employeesData ? (
                    <table  className="table-auto border-collapse w-full">
                        <thead>
                            <tr className="bg-gray-800 text-white text-xl lg:text-3xl">
                                <th className="text-left md:text-center py-2 px-4">Name</th>
                        ...
                        <tbody className='text-white text-xl text-center'>
                            {employeesData.map((employee) => (
                                <tr key={employee._id} className="border-b border-gray-300">
                                    <td className="text-left md:text-center py-2 md:pl-2 px-4 md:px-0">{employee.name}</td>
